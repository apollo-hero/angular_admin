...
export class AuthService {
  ...
  constructor(
    ...
    private _authMocApi: AuthMockApi
  ) {
  ...
  signIn(credentials: { username: string; password: string }): Observable<any> {
    ...
    this._authMocApi.registerHandlers();
  ...
  check(): Observable<0 | 1 | 2> {
    ...
    console.log(this.kq_LoggedIn,"login",this.kq_Token,AuthUtils.isTokenExpired(this.kq_Token, this.kq_TokenExpires));
    if (!this.kq_LoggedIn) {
      this.kqLoginClear();
      return of(0);
    }
